...
@section('content')
    ...
    <x-level-two.container :value-object="$valueObject">
        ...
        <x-level-two.block>
            ...
            <x-slot:value>{{$valueObject->phoneNumber}}</x-slot>
        </x-level-two.block>

        <x-level-two.block>
            <x-slot:title>Atbildīgās personas e-pasta adrese:</x-slot>
            <x-slot:value>{{$valueObject->email}}</x-slot>
        </x-level-two.block>

        <x-level-two.block>
            <x-slot:title>Atbildīgās personas vārds, uzvārds::</x-slot>
            <x-slot:value>{{$valueObject->playgroundResponsibleFullName}}</x-slot>
        </x-level-two.block>

        <x-level-two.block>
            <x-slot:title>Laukuma nosaukums:</x-slot>
            <x-slot:value>{{$valueObject->name}}</x-slot>
        </x-level-two.block>

        <x-level-two.block>
            <x-slot:title>Laukuma veids:</x-slot>
            <x-slot:value>{{$valueObject->isIndoor ? 'Iekštelpu' : 'Ārtelpu'}}</x-slot>
        </x-level-two.block>
