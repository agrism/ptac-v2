            ...
            @foreach($items as $item)
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 lg:grid-cols-[36%_36%_26%] gap-4 p-4 border-b hover:bg-gray-50 border-b-2 border-baseblue-400 lg:border-b-gray-200">
                    <div class="md:order-1 lg:order-1">
                        ...
                        <div><x-tooltip text="->ptacItemName">{{$item->ptacItemName}}</x-tooltip></div>
                    </div>
                    <div class="md:order-2 lg:order-2">
                        ...
                        <x-level-one.info-block>
                            <x-level-one.info-block-item>
                                ...
                                <x-slot:value><x-tooltip text="->ptacItemInformation">{{$item->ptacItemInformation}}</x-tooltip></x-slot:value>
                            </x-level-one.info-block-item>
                            <x-level-one.info-block-item>
                                <x-slot:label>Preces vai pakalpojuma klasifikācija</x-slot:label>
                                <x-slot:value><x-tooltip text="->ptacItemGroup">{{$item->ptacItemGroup ?: '-'}}</x-tooltip></x-slot:value>
                            </x-level-one.info-block-item>
                            <x-level-one.info-block-item>
                                <x-slot:label>Konstatētā neatbilstība un riski</x-slot:label>
                                <x-slot:value><x-tooltip text="->inconsistenciesAndRisks">{{$item->inconsistenciesAndRisks ?: '-'}}</x-tooltip></x-slot:value>
                            </x-level-one.info-block-item>
                            <x-level-one.info-block-item>
                                <x-slot:label>Riski</x-slot:label>
                                <x-slot:value><x-tooltip text="->330"><span class="bg-yellow-200">-</span></x-tooltip></x-slot:value>
                            </x-level-one.info-block-item>
                        </x-level-one.info-block>
                    ...
                    <div class="md:order-1 lg:order-3">
                        ...
                        >
                            <div class="px-2 py-1">
                                <x-tooltip text="->ptacItemModel">{{$item->ptacItemModel}}</x-tooltip>
                            </div>
                        </x-registers.index-decision>
                    </div>
                </div>
